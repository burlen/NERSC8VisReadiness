...
vtkStandardNewMacro(vtkN8ContourFilter);
vtkCxxSetObjectMacro(vtkN8ContourFilter,ScalarTree,vtkScalarTree);

// Construct object with initial range (0,1) and single contour value
// of 0.0.
vtkN8ContourFilter::vtkN8ContourFilter()
{
...
unsigned long vtkN8ContourFilter::GetMTime()
{
...
int vtkN8ContourFilter::RequestUpdateExtent(vtkInformation* request,
...
//
int vtkN8ContourFilter::RequestData(
  ...
  vtkInformationVector* outputVector)
{
  ...
  if (globalLogLevel)
    {
    log->StartEvent("vtkN8ContourFilter::RequestData");
    }
  ...
  if (vtkImageData::SafeDownCast(input) && sType != VTK_BIT &&
    ...
    if ( dim == 2 )
      ...
      if (globalLogLevel)
        {
        log->EndEvent("vtkN8ContourFilter::RequestData");
        }
      return status;
      }
    else if ( dim == 3 )
  ...
  if (vtkRectilinearGrid::SafeDownCast(input) && sType != VTK_BIT)
    ...
    if (uExt[0] < uExt[1] && uExt[2] < uExt[3] && uExt[4] < uExt[5])
      ...
      this->RectilinearSynchronizedTemplates->SetComputeGradients(this->ComputeGradients);
      this->RectilinearSynchronizedTemplates->SetComputeScalars(this->ComputeScalars);
      this->RectilinearSynchronizedTemplates->SetInputArrayToProcess(0,this->GetInputArrayInformation(0));

      int status
        = this->RectilinearSynchronizedTemplates->ProcessRequest(request,inputVector,outputVector);
  ...
  if (vtkStructuredGrid::SafeDownCast(input) && sType != VTK_BIT)
    ...
    if (uExt[0] < uExt[1] && uExt[2] < uExt[3] && uExt[4] < uExt[5])
      ...
      int status
        = this->GridSynchronizedTemplates->ProcessRequest(request,inputVector,outputVector);
...
void vtkN8ContourFilter::SetLocator(vtkIncrementalPointLocator *locator)
{
...
void vtkN8ContourFilter::CreateDefaultLocator()
{
  if ( this->Locator == NULL )
    {
...
void vtkN8ContourFilter::SetArrayComponent( int comp )
{
...
int vtkN8ContourFilter::GetArrayComponent()
{
  return( this->SynchronizedTemplates2D->GetArrayComponent() );
}

void vtkN8ContourFilter::SetOutputPointsPrecision(int precision)
{
  this->OutputPointsPrecision = precision;
  this->Modified();
}

int vtkN8ContourFilter::GetOutputPointsPrecision() const
{
  return this->OutputPointsPrecision;
}

//----------------------------------------------------------------------------
int vtkN8ContourFilter::ProcessRequest(vtkInformation* request,
                                     vtkInformationVector** inputVector,
                                     vtkInformationVector* outputVector)
{
  // generate the data
...
int vtkN8ContourFilter::FillInputPortInformation(int, vtkInformation *info)
{
  info->Set(vtkAlgorithm::INPUT_REQUIRED_DATA_TYPE(), "vtkDataSet");
  return 1;
}

void vtkN8ContourFilter::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void vtkN8ContourFilter::ReportReferences(vtkGarbageCollector* collector)
{
